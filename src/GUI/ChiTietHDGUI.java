 ...
 * @author ntloc
 */
public class ChiTietHDGUI extends JDialog{
    
    private JLabel lbCTHD, lbMasp, lbTensp, lbDongia, lbSl, lbThanhtien, lbGiamgia, lbTongtien;
    ...
    public void Init(String mahd)
    {
        ...
        //----------------------------JPANEL TOP-------------------------------//
            ...
            pTop.setBackground(new Color(85,93,110));
                ...
                lbCTHD = new JLabel("CHI TIẾT HÓA ĐƠN: "+mahd,JLabel.CENTER);
                lbCTHD.setBounds(new Rectangle(380,12, 300, 30));
                lbCTHD.setForeground(Color.WHITE);
                lbCTHD.setFont(info);
            pTop.add(lbCTHD);
        //---------------------------JPANNEL CENTER-----------------------------//    
            ...
            pCenter.setBackground(Color.WHITE);
                ...
                DanhSachCTHD(mahd); //Load ChiTietHD(Mahd)
        ...
        //--------------------------EVENT JPANEL CENTER----------------------------------//  
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                else
                {
                    ...
                    if(option == 0)
                    {
                        ...
                        ChiTietHDDTO cthd = new ChiTietHDDTO(macthd, masp, tensp, dongia,sl,thanhtien,giamgia,tongtiencthd);

                        cthdBUS.Delete_CTHD(cthd);
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                else        //Sửa CTHD
                {
                    ...
                    String value_sp = table.getModel().getValueAt(row, column_masp).toString(); //get masp trong table muốn sửa
                    int value_sl = (int) table.getModel().getValueAt(row, column_sl);
                    if(masp.equals(value_sp)) //Nếu cùng mãsp và sửa số lượng
                    {
                        
                        SanPhamDTO sp = spBUS.Search(masp);
                        spBUS.Set_SLSP(masp,(sp.getSl() + value_sl));
                        spBUS.Set_SLSP(masp,((sp.getSl() + value_sl) - sl));

                        cthdBUS.Set(cthd,value_sp);    //Update CTHD
                    }
                    else //Nếu sửa sang mã sp khác
                    {
                        cthdBUS.Delete_Mahd_Masp(mahd,value_sp); //Xóa CTHD this.mãsp
                        cthdBUS.Add(cthd); //Thêm CTHD mãsp mới
                    }
    ...
    public void DanhSachCTHD(String mahd) //Load DanhSachSanPham lên table
    {   
        cthdBUS.DanhSachCTHD();
        ArrayList<ChiTietHDDTO> ds = cthdBUS.getDscthd(); //get Data 
        ArrayList<ChiTietHDDTO> dscthd = new ArrayList<>();
        for(ChiTietHDDTO cthd : ds)
        {
    ...
    {
        int tongtienhd = 0;
        ArrayList<ChiTietHDDTO> dscthd = cthdBUS.getDscthd();
